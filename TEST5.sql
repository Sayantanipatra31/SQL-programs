...
SELECT genre , AVG(rating) AS rating FROM reviews
JOIN series ON reviews.series_id=series.id
GROUP BY genre;

SELECT first_name,last_name , IFNULL(COUNT(rating),0) AS COUNT,IFNULL(MIN(rating), 0) AS MIN,
...
SELECT title,rating,CONCAT(first_name,' ',last_name) AS names FROM reviews
JOIN series ON reviews.series_id=series.id
JOIN reviewers ON reviews.reviewer_id=reviewers.id
LIMIT 10;